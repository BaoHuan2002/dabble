...
Sau đó tạo keyspace và table cho Cassandra:

docker exec -it dabble_cassandra cqlsh -u cassandra -p cassandra123 -k dabble_messenger -e "
CREATE TABLE IF NOT EXISTS messages (
    ...
    PRIMARY KEY ((conversation_id), created_date, id)
) WITH CLUSTERING ORDER BY (created_date DESC, id ASC);"
...
Đi tới thư mục src/main/resources/application.properties và điền các thông tin bảo mật còn thiếu (Các thông số Database đã được cấu hình sẵn khớp với Docker):
...
🛡️ Cloudflare Turnstile: Cung cấp Turnstile_SecretKey.
